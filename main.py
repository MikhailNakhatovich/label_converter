...
if __name__ == '__main__':
    ...
    if os.path.isfile(input_path) and input_path[-len(FILE_FILTER):].lower() == FILE_FILTER:
        convert(input_path, create_xml_path(input_path, output_path), easy_converter, rect)
    elif not args.tree:
        files = os.listdir(input_path)
        for _ in filter(lambda x: x[-len(FILE_FILTER):].lower() == FILE_FILTER, files):
            convert(os.path.join(input_path, _), create_xml_path(_, output_path), easy_converter, rect)
    else:
        tree = os.walk(input_path)
        for _ in tree:
            if not _[1]:
                for file in filter(lambda x: x[-len(FILE_FILTER):].lower() == FILE_FILTER, _[2]):
                    convert(os.path.join(_[0], file), create_xml_path(file, output_path), easy_converter, rect)
